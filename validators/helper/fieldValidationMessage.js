const { get, replace, startCase, defaults } = require('lodash');
...
class FieldValidationMessage {
    ...
    static get(rule, attrbName, options = {} ) {
        ...
        if (values) {
            ruleMessage.replace(ruleMessage, ':values', fields);
        ...
        if (max) {
            ...
            ruleMessage = `${ruleMessage} ${max}`.trim();
        }
...
module.exports =  FieldValidationMessage.get;
